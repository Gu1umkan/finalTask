...
create table agencies(id serial primary key , name varchar,phone_number varchar);
alter table agencies add  address_id int references addresses(id);

create table addresses(id serial primary key ,city varchar , region varchar, street varchar);
...
create table owners(id serial primary key ,first_name varchar, last_name varchar,email varchar unique , date_of_birth date,gender genders);

create table houses(id serial primary key , house_type HouseType,price numeric,rating float,description varchar,room int ,furniture boolean,address_id int references addresses(id),owner_id int references owners(id));

create table customers (id serial primary key ,first_name varchar, last_name varchar , email varchar unique,date_of_birth date, gender genders,nationality varchar,marital_status FamilyStatus);
create table rent_info(id serial primary key , owner_id int references owners(id), customer_id int references customers(id),agency_id int references agencies(id), house_id int references houses(id),check_in date , check_out date);
...
select o.first_name , h.* from owners o inner join houses h on o.id = h.owner_id where length(first_name) = max (select length(first_name) from owners );
--
-- - 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select id,
       case when price between  1500 and 2000 then true
            else false
           end as between_price
from houses;

-- - 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select h.* , a.* from houses h inner join addresses a on address_id = a.id where a.id in (5,6,7,8,9,10);
-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h.*, o.first_name, a.first_name  as agency_name, c.first_name from houses h inner join  owners o on h.owner_id = o.id inner join agencies a on a.id = h.;
...
select houses.house_type, houses.rating,o.first_name as owner_name from houses inner join owners o on o.id = houses.owner_id   order by rating desc;
...
select a.name , ad.*,h.id  as house_address from agencies a
                                                     inner join addresses ad on ad.id = a.address_id
                                                     inner join rent_info ri on a.id = ri.agency_id
                                                     inner join houses h on ad.id = h.address_id where a.name = 'My_house';
-- - 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select h.id as house_id , h.house_type, o.first_name as owner_name, a.* from  houses h inner join owners o on o.id = owner_id
                                                                                       inner join addresses a on a.id = h.address_id where furniture = true;

-- - 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.

select h.id as house_id , h.house_type,a.name from houses h
...
select h.* ,c.* from houses h left join  rent_info ri on h.id = ri.house_id
                              left join customers c on ri.customer_id = c.id  where customer_id  is null;
...
select o.first_name, c.first_name from owners o inner join rent_info ri on o.id = ri.owner_id
                                                inner join customers c on c.id = ri.customer_id where o.first_name like 'A%';

-- - 16.Эн уйу коп owner_ди жана анын уйлорунун санын чыгар.
select o.first_name,o.last_name from houses h inner join owners o on o.id = h.owner_id
where owner_id = (select owner_id from houses count(*)  group by owner_id);
...
select h.id, h.house_type , c.first_name as customer_name , a.city from houses h inner join rent_info ri on h.id = ri.house_id
                                                                                 inner join customers c on c.id = ri.customer_id
                                                                                 inner join addresses a on a.id = h.address_id where a.city = 'Bishkek';
...
select h.id, h.house_type , o.first_name as owner_name from houses h
                                                                inner join owners  o on h.owner_id = o.id
where h.id = (select id from houses
              where price =  (select max(price) from houses ));
